...
#'
#'
#' @param nlmeFormula Formula to be passed to nlme
...
#' @param randomFormula Formula to be passed to random= in lme() or nlme()
...
longRPart2 <- function(nlmeFormula,
                       method,
                      rPartFormula,
                      randomFormula,
                      data,
    ...
    responseName = attr(terms(getResponseFormula(nlmeFormula)),"term.labels")
    groupingFactor = data[,names(data)==groupingName]
    terms = attr(terms(nlmeFormula),"term.labels")
    ...
    evaluation <- function(y, wt, parms){
      model = lme(nlmeFormula,data=parms[groupingFactor%in%y,],
    ...
    split <- function(y, wt, x, parms, continuous){
      ...
      dev = vector()
      xUnique = unique(x)
      rootDev = lme(nlmeFormula,data=parms[groupingFactor%in%y,],
      ...
      if(continuous){
        ...
        for(i in xUnique){
          ...
          if(length(yLeft) < control$minbucket || length(yRight) < control$minbucket){
            ...
            modelLeft = try(lme(nlmeFormula,data=parms[groupingFactor%in%yLeft,],
                                random=randomFormula,correlation=R,na.action=na.omit),silent=TRUE)
            modelRight = try(lme(nlmeFormula,data=parms[groupingFactor%in%yRight,],
                                 random=randomFormula,correlation=R,na.action=na.omit),silent=TRUE)
    ...
    model = rpart(paste(groupingName,c(rPartFormula)),
                  method=list(eval=evaluation,split=split,init=initialize),control=control,data=data,parms=data)
    model$lmeModel = lme(nlmeFormula,data=data,
                         random=randomFormula,correlation=R,na.action=na.omit)
    model$nlmeFormula = nlmeFormula
