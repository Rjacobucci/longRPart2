lrpPlot <- function(model,smoothing="n",color=NULL,place="bottomright"){
  ...
  terms = attr(terms(model$nlmeFormula),"term.labels")
  ...
  for(i in 1:length(nodes)){
    ...
    mod = lme(model$nlmeFormula,data=dat,random=model$randomFormula,correlation=model$R,na.action=na.omit)
    if(length(terms)>1){
...
#
#
# A standard tree plot with line plots at the nodes
#
#
lrpTreePlot <- function(model,use.n=TRUE,colors=NULL,place="bottomright"){
  indexes = rpartco(model)
  confounding = (length(model$nlmeModel$contrasts)>1)
